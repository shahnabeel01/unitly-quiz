...
/* ═══════════════════════════════════════════════════════════════
   QUIZ ENGINE  +  LOCAL STORAGE PERSISTENCE
═══════════════════════════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  /* ── Storage keys ─────────────────────────── */
  var LS_PREFIX        = 'examprep_uw_';          // namespace
  var LS_ACTIVE_UNIT   = LS_PREFIX + 'activeUnit';
  var LS_UNITS_DONE    = LS_PREFIX + 'unitsDone';

  /* Returns the localStorage key for a specific unit's progress */
  function lsUnitKey(idx) {
    return LS_PREFIX + 'unit_' + idx;
  }
  ...
  /**
   * Persist the current unit's in-progress state.
   * Called after every meaningful state change.
   */
  function saveUnitProgress() {
    if (state.activeUnit === null) return;

    /* Only save if the quiz is still in-progress (not submitted) */
    var quizEl = $('uwQuiz');
    if (!quizEl || quizEl.classList.contains('uw-hidden')) return;
    ...
    try {
      ...
      localStorage.setItem(lsUnitKey(state.activeUnit), JSON.stringify(data));

      /* Remember which unit was last open */
      localStorage.setItem(LS_ACTIVE_UNIT, String(state.activeUnit));
    } catch (e) {
  ...
  /**
   * Load saved progress for a specific unit index.
   * Returns the saved object or null if nothing is stored.
   */
  function loadUnitProgress(idx) {
    try {
      var raw = localStorage.getItem(lsUnitKey(idx));
      if (!raw) return null;
      var data = JSON.parse(raw);

      /* Basic validation — make sure arrays exist and lengths match */
      var unit = UNITS[idx];
      if (!unit) return null;

      var total = unit.questions.length;
      if (
        !Array.isArray(data.answers)  || data.answers.length  !== total ||
        !Array.isArray(data.statuses) || data.statuses.length !== total ||
        typeof data.currentQ !== 'number'
      ) {
        /* Saved data is stale / corrupted — discard it */
        localStorage.removeItem(lsUnitKey(idx));
        return null;
      }
  ...
  /**
   * Clear saved progress for a specific unit.
   * Called when the user clicks Reset.
   */
  function clearUnitProgress(idx) {
    try {
      localStorage.removeItem(lsUnitKey(idx));
    } catch (e) { /* silent */ }
  }
  ...
  /**
   * Save the unitsDone map (overall progress across all units).
   */
  function saveUnitsDone() {
    try {
      localStorage.setItem(LS_UNITS_DONE, JSON.stringify(state.unitsDone));
    } catch (e) { /* silent */ }
  }

  /**
   * Restore the unitsDone map on page load.
   */
  function restoreUnitsDone() {
    try {
      var raw = localStorage.getItem(LS_UNITS_DONE);
      if (!raw) return;
      var parsed = JSON.parse(raw);
      if (parsed && typeof parsed === 'object') {
        state.unitsDone = parsed;
      }
    } catch (e) {
      console.warn('⚠️ unitsDone restore failed:', e);
    }
  }

  /**
   * On page load: if there was a unit open last time, restore it.
   */
  function restoreSession() {
    try {
      var raw = localStorage.getItem(LS_ACTIVE_UNIT);
      if (raw === null) return;                     // nothing saved
      var idx = parseInt(raw, 10);
      if (isNaN(idx) || idx < 0 || idx >= UNITS.length) return;

      var saved = loadUnitProgress(idx);
      if (!saved) return;                           // no in-progress data

      /* Only restore if the quiz wasn't finished
         (i.e. at least one question is still unanswered) */
      var hasUnfinished = saved.statuses.some(function (s) {
        return s === 'unanswered';
      });

      /* Even if all are answered we restore so user can review,
         unless the result screen was already shown (no way to detect
         that reliably, so we always restore in-progress state). */
      console.log('♻️ Restoring session for unit index ' + idx);
      loadUnit(idx, saved);   // pass saved snapshot to loadUnit
    } catch (e) {
  ...
  function init() {
    ...
    /* Restore overall progress first */
    restoreUnitsDone();
    ...
    /* Try to restore the last open unit */
    restoreSession();
  ...
  function buildSidebar() {
    ...
    UNITS.forEach(function (unit, idx) {
      ...
      /* Show a small dot indicator if this unit has saved progress */
      var hasSaved  = loadUnitProgress(idx) !== null;
      var isDone    = !!state.unitsDone[idx];
      var badgeHtml = '';

      if (isDone) {
        /* Unit was submitted — show score badge */
        var ud      = state.unitsDone[idx];
        var score   = Math.round((ud.correct / ud.total) * 100);
        badgeHtml   =
          '<span class="uw-unit-saved-badge uw-unit-done-badge" title="Completed · ' + score + '%">' +
            score + '%' +
          '</span>';
      } else if (hasSaved) {
        /* Unit has in-progress saved data */
        badgeHtml =
          '<span class="uw-unit-saved-badge" title="In progress — progress saved">●</span>';
      }

      li.innerHTML =
        '<span class="uw-unit-num">' + (idx + 1) + '</span>' +
        '<span class="uw-unit-info">' +
          '<span class="uw-unit-name">' + escHtml(unit.name) + badgeHtml + '</span>' +
    ...
    /* Inject tiny badge styles once (avoids needing HTML changes) */
    injectBadgeStyles();
  }

  /**
   * Inject the sidebar badge CSS once into <head>.
   * This is the ONLY place we touch styles, and we do it from JS
   * so we never touch the HTML file.
   */
  function injectBadgeStyles() {
    if (document.getElementById('uw-ls-badge-styles')) return; // already injected

    var style = document.createElement('style');
    style.id  = 'uw-ls-badge-styles';
    style.textContent = [
      /* In-progress dot badge */
      '.uw-unit-saved-badge {',
      '  display: inline-flex;',
      '  align-items: center;',
      ...
      '  font-size: 9px;',
      '  font-weight: 700;',
      '  color: #f5a623;',        /* amber — matches --amber */
      ...
      '.uw-unit-done-badge {',
      '  background: rgba(52,200,122,.15);',
      '  border: 1px solid rgba(52,200,122,.30);',
      '  color: #34c87a;',        /* --green */
      '  border-radius: 999px;',
      '  padding: 1px 6px;',
      '  font-size: 9.5px;',
      '}',
      /* Active item — keep badge readable */
      '.uw-unit-item.is-active .uw-unit-saved-badge:not(.uw-unit-done-badge) {',
      '  color: #6b93fa;',        /* --blue-light */
      '}'
    ].join('\n');
  ...
  /* ══════════════════════════════════════════
     LOAD UNIT
     @param {number}      idx    — unit index
     @param {object|null} saved  — optional snapshot from localStorage
  ══════════════════════════════════════════ */
  function loadUnit(idx, saved) {
    var unit = UNITS[idx];
    if (!unit) return;
    ...
    if (saved) {
      ...
      console.log('♻️ Restored unit ' + idx + ' at question ' + (saved.currentQ + 1));
    } else {
      /* ── FRESH start ── */
      state.currentQ = 0;
    ...
    /* Save the active-unit pointer immediately */
    try {
      localStorage.setItem(LS_ACTIVE_UNIT, String(idx));
    } catch (e) { /* silent */ }
  }
  ...
  function goPrevious() {
    if (state.currentQ > 0) {
      state.currentQ--;
      renderQuestion();
      saveUnitProgress(); /* 💾 Save current question position */
    }
  }
  ...
  function resetQuiz() {
    if (confirm('Reset this unit? All progress will be lost.')) {
      /* 🗑️ Clear localStorage for this unit */
      clearUnitProgress(state.activeUnit);

      /* Also clear the active-unit pointer so a fresh page load
         goes to welcome screen instead of restoring this unit */
      try { localStorage.removeItem(LS_ACTIVE_UNIT); } catch (e) { /* silent */ }

      loadUnit(state.activeUnit); /* fresh start — no saved snapshot passed */
      buildSidebar();             /* refresh badges in sidebar */
    }
  }
    ...
    for (var i = 0; i < total; i++) {
      (function (qi) {
        ...
        btn.addEventListener('click', function () {
          state.currentQ = qi;
          renderQuestion();
          saveUnitProgress(); /* 💾 Save when jumping via palette */
        });
  ...
  function syncMobilePalette() {
    ...
    btns.forEach(function (btn) {
      ...
      clone.addEventListener('click', function () {
        state.currentQ = qi;
        renderQuestion();
        saveUnitProgress(); /* 💾 Save when jumping via mobile palette */
        closePaletteDrawer();
      });
  ...
  function submitQuiz() {
    ...
    /* 💾 Save unitsDone + clear the in-progress snapshot
         (quiz is now finished — no need to restore mid-quiz state) */
    saveUnitsDone();
    clearUnitProgress(state.activeUnit);

    /* Also remove the active-unit pointer so a fresh page-load
       goes to the welcome screen (not back into a finished quiz) */
    try { localStorage.removeItem(LS_ACTIVE_UNIT); } catch (e) { /* silent */ }

    /* Rebuild sidebar so badges update (in-progress → score %) */
    buildSidebar();
    $$('.uw-unit-item').forEach(function (el, i) {
      el.classList.toggle('is-active', i === state.activeUnit);
    });
    ...
    /* Hide topbar palette btn & unit tag on result screen */
    var paletteBtn = $('paletteOpenBtn');
  ...
  function attachQuizListeners() {
    ...
    $('retryBtn') && $('retryBtn').addEventListener('click', function () {
      /* Retry = clear saved progress for this unit and restart fresh */
      clearUnitProgress(state.activeUnit);
    ...
    $('nextUnitBtn') && $('nextUnitBtn').addEventListener('click', function () {
      ...
      if (next < UNITS.length) {
        /* Check if the next unit has saved in-progress data */
        var saved = loadUnitProgress(next);
        loadUnit(next, saved || undefined);
      }
    });
  ...
  function attachDrawerListeners() {
    ...
    if (sidebar) {
      ...
      sidebar.addEventListener('touchend', function (e) {
        if (e.changedTouches[0].clientX - swipeStartX < -60) closeSidebar();
      }, { passive: true });
    }
    ...
    if (drawer) {
      ...
      drawer.addEventListener('touchend', function (e) {
        if (e.changedTouches[0].clientY - swipeStartY > 60) closePaletteDrawer();
      }, { passive: true });
    }
